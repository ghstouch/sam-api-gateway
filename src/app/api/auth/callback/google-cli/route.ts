...
export async function GET(req: NextRequest) {
  ...
  const provider = PROVIDERS['google-cli'];
  if (!provider?.oauth) {
  ...
  try {
    ...
    return new NextResponse(`
      <html><body style="background:#1a1a2e;color:#eee;font-family:sans-serif;text-align:center;padding:50px">
        ...
        <script>
          if (window.opener) { window.opener.postMessage({ type: 'oauth-connected', provider: 'google-cli' }, '*'); }
